...
import { addFavoriteItem, removeFavoriteItem } from "../redux/favSlice";
  ...
  const handleClick = async () => {
    ...
    if (!hasClicked) {
      ...
      await updateDoc(userDocRef, {
        favItems: arrayUnion(product),
      });
    } else {
      dispatch(removeFavoriteItem(product.id));
      await updateDoc(userDocRef, {
        favItems: arrayRemove(product),
      });
    }
  };

  return (
    <div className="flex mt-0.5 items-center justify-end" id={product.id}>
      <motion.div
        whileTap={{ scale: 3 }}
